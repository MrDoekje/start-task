  ...
  if (!existsSync(projectDir)) {
    throw new Error(
      `Project directory does not exist: ${projectDir}\n` +
        `Clone the repository into your workspace before running this flow.`,
    );
  }
 ...
 * @param {string} [startPoint] - Git ref to base the new branch on (defaults to `origin/${branchName}`)
 * @returns {string} Path to the worktree directory
 */
export function ensureWorktree(projectDir, branchName, worktreeDir, startPoint) {
  if (!existsSync(projectDir)) {
    throw new Error(
      `Project directory does not exist: ${projectDir}\n` +
        `Clone the repository into your workspace before running this flow.`,
  ...
  const ref = startPoint ?? `origin/${branchName}`;

  try {
    git(`worktree add -b ${branchName} "${worktreeDir}" ${ref}`, projectDir);
  ...
  git(`config branch.${branchName}.remote origin`, worktreeDir);
  git(`config branch.${branchName}.merge refs/heads/${branchName}`, worktreeDir);
