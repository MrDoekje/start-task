 ...
 * @property {Record<string, { submenu?: boolean }>} [groups] - Optional per-group menu settings keyed by the `flow.group` label. `submenu: true` collapses the group into a drill-down entry; omitted or false keeps flows inline under a separator header.
 * @property {false | { enabled?: boolean, count?: number }} [frequentlyUsed] - Controls the "Frequently Used" section at the top of the main menu. Defaults to enabled with `count: 3`. Pass `false` or `{ enabled: false }` to disable. Usage is tracked in `~/.start-task/usage.json`.
 */
 ...
 * @property {string} [group] - Optional menu group label. Flows sharing a group are rendered under a non-selectable header; ungrouped flows fall under "Other".
 ...
 * @property {"text" | "multiline" | "editor" | "multiselect" | "select"} type - Prompt step type ("multiline" is an in-app multi-line input; "editor" opens $EDITOR)
 ...
 * @property {((value: string) => string)} [clean] - For text steps: derive a sanitized suggestion shown below the input; Tab applies it. Suggestion is hidden when empty or equal to the trimmed value.
 ...
 * @property {((results: Record<string, any>, config: Config) => boolean)} [when] - Conditional function; step is skipped when it returns false
 */
 ...
 * @property {"text" | "multiline" | "editor" | "multiselect" | "select"} type - Prompt step type
