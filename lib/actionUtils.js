...
export function buildActionUtils(config) {
   ...
   * @param {string} [options.startPoint] - Git ref to base a new branch on (defaults to `origin/${branchName}`)
   * @returns {string}
   */
  function ensureWorktree(projectDir, branchName, options) {
    const worktreeDir =
      config.worktree?.path?.(projectDir, branchName) ??
      resolve(projectDir, "worktrees", branchName);
    return ensureWorktreeRaw(projectDir, branchName, worktreeDir, options?.startPoint);
  }
