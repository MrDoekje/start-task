...
export function buildActionUtils(config) {
   ...
   * so flow actions call launchTask(dirs, prompt, name) with 3 args unchanged.
   * @param {string | string[]} workingDirs
   * @param {string} prompt
   * @param {string} windowName
   */
  function launchTask(workingDirs, prompt, windowName) {
    config.sessionManager.launchTask(workingDirs, prompt, windowName, config.agent.buildCommand);
  }
