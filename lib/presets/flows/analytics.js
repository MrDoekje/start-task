/**
 * Agent filter — only shown when more than one analytics provider is
 * configured. `when` receives config so it can inspect the provider count.
 * @type {import("../../types.js").WizardStep}
 */
const agentFilterStep = {
  type: "multiselect",
  key: "agentFilter",
  message: "Which agents to include?",
  required: true,
  when: (_results, config) => normalizeProviders(config.analytics).length > 1,
  options: (config) =>
    normalizeProviders(config.analytics).map((prov) => ({
      value: prov.name,
      label: prov.name,
    })),
};
...
export const analyticsFlow = {
  label: "Analytics",
  steps: [agentFilterStep, timeRangeStep, groupByStep, reportSectionsStep],
...
/**
 * Prints a single informational line, preceded by a blank line.
...
/**
 * Prints a titled section: a heading, an underline, then the indented body.
 * Replaces the bordered note boxes the wizard used to render via clack.
 * @param {string} body
 * @param {string} title
 */
...
function printReport(result, sections, perAgentResults) {
  ...
  info(`Period: ${from} to ${to} | ${result.providerName}`);
  ...
  if (sections.has("tools")) {
    if (result.tools.length === 0) {
      note("No tool usage found.", "Tool usage");
    } else {
  ...
  if (sections.has("subagents")) {
    if (!result.subagents || result.subagents.length === 0) {
      note("No subagent usage found.", "Subagent usage");
    } else {
  ...
  if (sections.has("skills")) {
    if (!result.skills || result.skills.length === 0) {
      note("No skill usage found.", "Skill usage");
    } else {
  ...
  if (sections.has("models")) {
    if (!result.models || result.models.length === 0) {
      note("No model data found.", "Model usage");
    } else {
  ...
  if (sections.has("projects")) {
    if (result.projects.length === 0) {
      note("No project data found.", "Project breakdown");
    } else {
  ...
  if (sections.has("sessions")) {
    ...
    if (top.length === 0) {
      note("No sessions found.", "Session patterns");
    } else {
