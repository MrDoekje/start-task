  ...
  function launchTask(workingDirs, prompt, windowName, buildCommand) {
    ...
    for (let i = 1; i < dirs.length; i++) {
      try {
        ...
        // No space left — collect remaining dirs for an overflow window
        overflowDirs = dirs.slice(i);
        break;
      }
    }

    // Create overflow window for dirs that didn't fit
    if (overflowDirs.length > 0) {
      const overflowName = `${safeName}-shells`;
      const overflowId = tmux(
        `new-window -t ${session} -n "${overflowName}" -c "${overflowDirs[0]}" -P -F "#{window_id}"`,
      );
      for (let i = 1; i < overflowDirs.length; i++) {
        try {
          tmux(`split-window -t ${overflowId} -v -c "${overflowDirs[i]}"`);
        } catch {
          break;
        }
      }
