...
async function wizard(steps) {
  ...
  while (i < steps.length) {
    ...
    if (p.isCancel(value)) {
      if (i === 0) return null;
      i--;
      // Skip back over conditional steps that were skipped
      while (i > 0 && steps[i].when && !steps[i].when(results)) {
        i--;
      }
      continue;
    }
...
export async function runFlow(flow, config, runtimeOptions) {
  ...
  const wizardSteps = resolvedSteps.map((step) => ({
    ...buildWizardStep(step, preWizardConfig, preWizardUtils),
    when: step.when,
  }));
...
export default async function runTui() {
  ...
  while (true) {
    ...
    const menuOptions = [
      ...
      { value: "__restart", label: "Restart TUI" },
    ...
    try {
      ...
      if (action === "__restart") {
        p.log.step("Restarting…");
        config = await loadConfig();
        utils = buildActionUtils(config);
        console.clear();
        p.intro("start-task TUI");
        continue;
      }
